...
** verview of the code flow

   during bluetooth init, we'll initializes the BT low power manager.
   
   #+BEGIN_SRC plantuml :file ../images/2018/2018081805.png :cmdline -charset UTF-8
     @startuml
     (*) --> bta_dm_sys_hw_cback
     bta_dm_sys_hw_cback --> bta_dm_init_pm
     bta_dm_init_pm --> [bta_dm_pm_cback] bta_sys_pm_register
     bta_dm_init_pm --> [bta_dm_pm_btm_cback] BTM_PmRegister
     @enduml
   #+END_SRC

   #+RESULTS:
   [[file:../images/2018/2018081805.png]]

   of the two callbacks:

   - bta_dm_pm_cback

     is called when connection status changed for system connection.

   - bta_dm_pm_btm_cback

     called when power management status changed. it will call
     *bta_dm_pm_btm_status* furhter to process the
     BTA_DM_PM_BTM_STATUS_EVT. 

   both the above functions may call call *bta_dm_pm_set_mode* to set the
     power mode inclding sniff mode. 

   Each profile will register with BT Power manager module during
   profile connection(through *bta_dm_pm_cback*). Each profile has default set sniff idle time
   out and other sniff parameters based on the *bta_dm_pm_cfg*.

   here is the definition of it's structure:

   #+BEGIN_SRC c
     typedef struct {
       uint8_t id;   /* profile role index in the BTA system.
       uint8_t app_id;
   ...
   [[../images/2018/2018081801.png]]

   take the AG module configuration for example, for AG module, the
   spec index is 0 in spec table *bta_dm_pm_spec*.
   ...
   [[../images/2018/2018081803.png]]

   If more than 1 profile connected on same ACL, then least time out
   of all connected profiles will be effective.

   Also, the SSR entry is *BTA_DM_PM_SSR2*, which means that the max
   latency is 1200 slots.

   [[../images/2018/2018081804.png]]
   ...
   These ssr parameters also depends on profile. Each profile set ssr
   parameter based on config table(bta_dm_pm_spec) we referred earlier.

   For most profile connection, when in SSR, the max latency is 1200 *
   0.625 = 750ms. Only HID profile will use different ssr
   parameter. When DUT  moves back to active mode from any other low
   power mode, DUT update ssr parameters to SoC.

   SoC will use these sniff sub rating parameters in sniff mode.

** setting sniff parameters -- bta_dm_pm_sniff

   this function is used to call *BTM_SetPowerMode* to enter sniff
   mode or exit sniff mode. 

   for different profiles, the sniff paramters are set differently. 

   for example, for a2dp profile, the sniff paramter is index by
   *BTA_DM_PM_SNIFF_A2DP_IDX* which is eaual to *BTA_DM_PM_SNIFF* . 

   this is the sniff table :
   ...
   and we have different sniff paramter settings with them:
   ...
   this is the power action defined for the profiles. 
   ...
   to index a specific sniff parameter settings for a power action in
   the array *bta_dm_pm_md* ,  use 
   : (pm_action & 0x0F)

** Sniff Mode常见的一些修改

   1. 建立Sco的时候本退出sniff mode

      #+BEGIN_SRC diff
        diff --git a/bta/dm/bta_dm_pm.cc b/bta/dm/bta_dm_pm.cc
        index 92a233e..926acf8 100644
        --- a/bta/dm/bta_dm_pm.cc
        +++ b/bta/dm/bta_dm_pm.cc
        @@ -483,7 +483,14 @@ static void bta_dm_pm_cback(tBTA_SYS_CONN_STATUS status, uint8_t id,
        }
        }
        }
        -
        + if ((status == BTA_SYS_SCO_OPEN || status == BTA_SYS_SCO_CLOSE)&&(id == BTA_ID_AG)) {
        ...
        + APPL_TRACE_DEBUG("%s: BTA_ID_AG with bScoActive = %d", __func__, bScoActive);
        + if(p_rem_dev)
   ...
   2. 切换到新的HFP设备时，退出sniff mode

      #+BEGIN_SRC diff
        diff --git a/bta/ag/bta_ag_sco.cc b/bta/ag/bta_ag_sco.cc
        index c8e67f4..22b7767 100644
        --- a/bta/ag/bta_ag_sco.cc
        +++ b/bta/ag/bta_ag_sco.cc
        @@ -2115,7 +2115,11 @@ void bta_ag_api_set_active_device(tBTA_AG_DATA* p_data) {
...
* FAQ
  ...
  1. What conditions may trigger host send sniff sub-rating command?
     How does the host set parameters? Is the 1200 latency the default
     value and not change?

     The short answer is host only set sniff subrate parameters, and
     it’s SoC which will negotiate with the remote device to go to
     sniff subrate mode further.  When sniff mode is initiated by our
     DUT,  this latency is by default 1200 slots for most of
     profiles.

  2. How to tell if link is in sniff subrating mode?
     
     When host send sniff subrate mode, the link doesn’t enter sniff
     subrating. Only when controller send sniff subrating mode event,
     it means the link enters sniff subrating mode. Here is the event:

     [[../images/2018/2018081806.png]]

  3. The conditions of enter sniff subrating mode, assuming
     initiated by DUT:

     1) Host sent sniff subrating command to set SSR parameters

        If the sniff subrating max latency is not bigger than 2 times
        of sniff interval, link will not enter sniff subrating mode.

     2) Link is in sniff mode;

     3) Link has no data for some time (sniff interval)

     4) DUT and remote device both agree to enter sniff subrating
        mode, which means *LMP_Sniff_Sub-rate_Req* and
        *LMP_Sniff_Sub-rate_Res* transactions are successful.
